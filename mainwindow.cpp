...
void MainWindow::creatMainWidget()
{
	...
	//analysisTree_ = new QTreeWidget();
	//analysisTreeDock_ = new QDockWidget("Select Widget", this);
	//analysisTreeDock_->setMinimumSize(200, 150);
	////QHBoxLayout* HorizontalLayout = new QHBoxLayout(analysisTreeDock_);
	////HorizontalLayout->addWidget(analysisTree_);
	//analysisTree_->setColumnCount(1);  //ÉèÖÃÁÐ
	//analysisTreeDock_->setWidget(analysisTree_);
	////analysisTree_->setHeaderLabels("item tree"));

	//addDockWidget(Qt::RightDockWidgetArea, new QDockWidget("Select Widget", this) );


	analysisTreeDock_ = new QDockWidget("hex analysis", this);
	hexEditDock_ = new QDockWidget("HexEdit", this);

	hexEditDock_->setWidget(hexEdit);
	hexEditDock_->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable);

	analysisTree_ = new QTreeWidget();

	analysisTreeDock_->setWidget(analysisTree_);
	analysisTreeDock_->setFeatures(QDockWidget::DockWidgetMovable | QDockWidget::DockWidgetFloatable);

	//QPushButton* btn_Play = new QPushButton("Play", mywid1);
	//btn_Play->setFixedSize(40, 30);
	addDockWidget(Qt::RightDockWidgetArea, analysisTreeDock_);
	addDockWidget(Qt::LeftDockWidgetArea, hexEditDock_);
	//this->setDockOptions()
